<div class="modal fade" id="AppointmentModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h3 class="modal-title">{{__('messages.make_appointment')}}</h3>
            ...
            <div class="modal-body">
                ...
                <div class="mb-5"> {{ Form::label('name',__('messages.common.name').(':'), ['class' => 'form-label required']) }} {{ Form::text('name', null, ['class' => 'form-control','required', 'placeholder' => __('messages.form.enter_name'),'id'=>'paypalIntUserName']) }} </div>
                <div class="mb-5"> {{ Form::label('email',__('messages.common.email').(':'), ['class' => 'form-label required ']) }} {{ Form::text('email', null, ['class' => 'form-control','required', 'placeholder' => __('messages.form.enter_email'),'id'=>'paypalIntUserEmail']) }} </div>
                <div class="mb-5"> {{ Form::label('phone',__('messages.common.phone').(':'), ['class' => 'form-label']) }} {{ Form::text('phone', null, ['class' => 'form-control', 'placeholder' => __('messages.form.enter_phone'),'id'=>'paypalIntUserPhone']) }} </div> @if(isset($appointmentDetail->is_paid) && ($appointmentDetail->is_paid == 1) && (getUserSettingValue('stripe_enable',$vcard->user->id) || getUserSettingValue('paypal_enable',$vcard->user->id)))
                    <div class="mb-5"> {{ Form::label('payment_method',__('messages.common.payment_methods').(':'), ['class' => 'form-label required']) }} {{ Form::select('payment_method', $appointmentDetail->is_paid == 0 ? \App\Models\Appointment::PAYMENT_METHOD :$paymentMethod , null,['class' => 'form-control form-select form-select-solid select2Selector','data-control' => 'select2', 'required', 'id' => 'appointmentPaymentMethod','placeholder'=>__('messages.common.payment_methods')]) }} </div>
                    <div class="mb-5"> {{ Form::label('phone',__('Price').(':'), ['class' => 'form-label']) }} <span
            ...
            <div class="modal-footer pt-0"> {{ Form::button(__('messages.common.save'), ['type'=>'submit','class' => 'btn btn-primary m-0','id'=>'serviceSave']) }}
