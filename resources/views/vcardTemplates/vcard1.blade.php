...
<div class="container">
    ...
    <div class="vcard-one main-content w-100 mx-auto content-blur allSection collapse show">
        ...
        <div class="vcard-one__profile-details py-3 d-flex flex-column align-items-center justify-content-center px-3">
            ...
            <span class="mt-4 profile-description"> {{$vcard->description}}</span>
                            ...
                            <div class="card event-card p-3 h-100 shadow-none border-0">
                             <span class="event-icon d-flex justify-content-center">
                                ...
                                <a href="tel:+{{ $vcard->region_code }}{{ $vcard->phone }}"
                                   class="event-name text-center pt-3 mb-0 text-decoration-none">+ {{ $vcard->region_code }} {{ $vcard->phone }}</a>
                            </div>
                        </div>
                    @endif
                             ...
                             <span class="event-icon d-flex justify-content-center">
                                <img src="{{asset('assets/img/vcard1/vcard1-alter-phone(4).png')}}" alt="email"/>
                                    ...
                                    <a href="tel:+{{ $vcard->region_code }} {{ $vcard->alternative_phone }}"
                                       class="event-name text-center pt-3 mb-0 text-decoration-none">+{{ $vcard->region_code }} {{ $vcard->alternative_phone }}</a>
                                </div>
                            </div>
                        @endif
                    @if($vcard->location)
...
{{--                    <span class="slick-next">&#8594;fsddsfsdfsd</span>--}}
        ...
        @if(checkFeature('blog') && $vcard->blogs->count())
            <div class="vcard-one__blog my-3 py-5">
       ...
       @if(checkFeature('advanced'))
           @if(checkFeature('advanced')->hide_branding && $vcard->branding == 0)
               @if($vcard->made_by)
                   <a @if(!is_null($vcard->made_by_url)) href="{{$vcard->made_by_url}}" @endif class="text-center text-decoration-none text-dark" target="_blank">
           ...
           @if(!empty($vcard->privacy_policy) || !empty($vcard->term_condition))
               <div>
                   <a class="text-decoration-none text-dark cursor-pointer" href="{{ route('vcard.show-privacy-policy',[$vcard->url_alias,$vcard->id]) }}"><small>{{__('messages.vcard.term_policy')}}</small></a>
               </div>
           @endif
       </div>
